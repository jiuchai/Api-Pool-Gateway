...
const redeemService = {
  ...
  async generateCodes(options, adminId) {
    const { count = 1, type, tierIndex, maxUses = 1, expiresAt } = options;
    ...
    for (let i = 0; i < count; i++) {
      ...
      const entry = { code, type, tierIndex: tierIndex || -1, maxUses: maxUses === -1 ? -1 : maxUses, usedCount: 0, expiresAt: expiresAt || null, disabled: false, createdBy: adminId, createdAt: Date.now() };
