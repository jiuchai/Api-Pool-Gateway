    ...
    <div class="tl">
      <div class="card rc">
        ...
        <div class="cb">
          ...
          <div class="fg" v-for="p in serviceParams" :key="p.name"><label>{{ p.name }} <span v-if="p.required" class="rq">*</span></label><input v-model="params[p.name]" class="fc" :placeholder="p.description" /></div>
...
<script setup>
import { ref, onMounted, watch } from 'vue'; import { get } from '@/api/client'; import { useRoute } from 'vue-router'; import { useToastStore } from '@/stores/toast'
const toast = useToastStore(); const route = useRoute()
const services = ref([]); const slug = ref(''); const apiKey = ref(''); const params = ref({}); const serviceParams = ref([]); const resp = ref(null); const rt = ref(0); const loading = ref(false); const err = ref('')
...
async function loadInfo() {
  ...
  try { const r = await get(`/api/gateway/${slug.value}/info`); serviceParams.value = r.data.data.params || []; params.value = {} }
...
async function send() {
  ...
  try {
    const res = await fetch(`/api/gateway/${slug.value}`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-API-Key': apiKey.value }, body: JSON.stringify(params.value) })
