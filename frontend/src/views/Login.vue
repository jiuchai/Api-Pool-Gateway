...
<script setup>
import { ref } from 'vue'; import { useRouter, useRoute } from 'vue-router'; import { useAuthStore } from '@/stores/auth'; import { useToastStore } from '@/stores/toast'
const r = useRouter(); const route = useRoute(); const auth = useAuthStore(); const toast = useToastStore()
const f = ref({ username: '', password: '' }); const ld = ref(false); const err = ref('')
async function go() { err.value = ''; ld.value = true; try { await auth.login(f.value); toast.success('登录成功'); r.push(route.query.redirect || '/dashboard') } catch(e) { err.value = e.message } finally { ld.value = false } }
</script>
<style scoped>
