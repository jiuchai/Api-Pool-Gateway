<template>
  <div class="page container">
    <h1 class="pt">全局日志</h1>
    <div class="fb">
      <el-input v-model="fl.username" placeholder="用户名" style="width:120px" size="default" />
    ...
    <div class="card"><div class="cb">
      <table v-if="logs.length"><thead><tr><th>时间</th><th>用户</th><th>方法</th><th>路径</th><th>服务</th><th>状态码</th><th>响应</th><th></th></tr></thead><tbody>
        <template v-for="l in logs" :key="l.id">
          <tr class="lr" :class="{ex:l._e}" @click="l._e=!l._e"><td class="time">{{ l.timestamp }}</td><td>{{ l.username }}</td><td><span class="mt">{{ l.method }}</span></td><td class="path">{{ l.path }}</td><td>{{ l.serviceName || l.serviceSlug || '-' }}</td><td><span :class="sc(l.statusCode)">{{ l.statusCode }}</span></td><td>{{ l.responseTime }}ms</td><td><span class="ei">{{ l._e?'▲':'▼' }}</span></td></tr>
          <tr v-if="l._e" class="ldr"><td colspan="8"><div class="ld">
            <div class="ds"><h4>基本信息</h4><div class="info-grid"><div><span class="il">方法:</span>{{ l.method }}</div><div><span class="il">路径:</span><code>{{ l.path }}</code></div><div><span class="il">状态码:</span>{{ l.statusCode }}</div><div><span class="il">响应时间:</span>{{ l.responseTime }}ms</div><div><span class="il">目标URL:</span><code>{{ l.targetUrl||'-' }}</code></div><div><span class="il">IP:</span>{{ l.ip||'-' }}</div></div></div>
...
const fl = reactive({ username:'',serviceSlug:'',status:'',startTime:'',endTime:'' })
...
.pt { font-size: 1.5rem; margin-bottom: 16px; }
.fb { display: flex; gap: 10px; margin-bottom: 16px; flex-wrap: wrap; align-items: center; }
.card { background: #fff; border: 1px solid #e2e8f0; border-radius: 10px; }
.cb { padding: 20px; overflow-x: auto; }
