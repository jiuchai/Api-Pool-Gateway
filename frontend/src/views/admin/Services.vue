<template>
  <div class="page container">
    ...
    <!-- 编辑弹窗 -->
    <el-dialog v-model="show" :title="editing?'编辑服务':'添加服务'" width="750px" destroy-on-close class="svc-dialog">
      <div class="mb">
        ...
        <div class="row2">
          ...
          <div class="fg"><label>分类</label><el-select v-model="form.category" style="width:100%" filterable allow-create clearable placeholder="选择或输入新分类"><el-option v-for="c in categories" :key="c" :label="c" :value="c" /></el-select></div>
        </div>
        <div class="fg"><label>描述</label><el-input v-model="form.description" type="textarea" :rows="2" placeholder="此服务的功能说明" /></div>
        <div class="row2">
          <div class="fg"><label>在线使用网址 <span class="hlp">（可选，设置后在工具中心显示"在线使用"按钮）</span></label><el-input v-model="form.welcomeUrl" placeholder="https://example.com/tool" /></div>
        </div>
        <div class="row2">
          <div class="fg"><label>目标URL <span class="hlp">（Docker用 host.docker.internal 替代 127.0.0.1）</span></label><el-input v-model="form.targetUrl" placeholder="https://api.example.com/endpoint" /></div>
        ...
        <div class="params-section">
          ...
          <div class="param-row" v-for="(p,i) in form.params" :key="i">
            ...
            <el-select v-model="p.type" style="width:100px" filterable allow-create clearable placeholder="类型">
              <el-option value="string" /><el-option value="number" /><el-option value="boolean" /><el-option value="file" /><el-option value="files" /><el-option value="list" />
            </el-select>
          ...
          <div class="param-row">
            ...
            <el-select v-model="newParam.type" style="width:100px" filterable clearable placeholder="类型">
              <el-option value="string" /><el-option value="number" /><el-option value="boolean" /><el-option value="file" /><el-option value="files" /><el-option value="list" />
            </el-select>
...
import SiteSettings from '@/views/admin/Settings.vue'
...
const form = ref({ name:'',slug:'',category:'',description:'',targetUrl:'',welcomeUrl:'',method:'POST',forwardType:'json',forwardMode:'template',customScript:'',bodyTemplate:'',forwardHeadersJson:'',params:[],inputExample:'',outputExample:'' })
const newParam = reactive({ name:'', type:'string', desc:'' })
...
function addParam() {
  ...
  form.value.params.push({ name:newParam.name, type:newParam.type || 'string', required:false, description:newParam.desc })
  newParam.name = ''; newParam.type = 'string'; newParam.desc = ''
}
async function load() { try { const r = await get('/api/admin/services'); services.value = r.data.data } catch {} }
function openCreate() { editing.value = false; form.value = { name:'',slug:'',category:'',description:'',targetUrl:'',welcomeUrl:'',method:'POST',forwardType:'json',forwardMode:'template',customScript:'',bodyTemplate:'',forwardHeadersJson:'',params:[],inputExample:'',outputExample:'' }; show.value = true }
function editService(s) {
  editing.value = true
  form.value = { name:s.name, slug:s.slug, category:s.category||'', description:s.description||'', targetUrl:s.targetUrl||'', welcomeUrl:s.welcomeUrl||'', method:s.method||'POST', forwardType: s.forwardType || 'json', forwardMode: s.forwardMode || 'template', customScript: s.customScript || '', bodyTemplate:s.bodyTemplate||'', forwardHeadersJson: s.forwardHeaders ? JSON.stringify(s.forwardHeaders) : '', params: s.params || [], inputExample: s.inputExample || '', outputExample: s.outputExample || '' }
  show.value = true
}
async function save() {
  const data = { name:form.value.name, slug:form.value.slug, category:form.value.category, description:form.value.description, targetUrl:form.value.targetUrl, welcomeUrl:form.value.welcomeUrl, method:form.value.method, forwardType:form.value.forwardType, forwardMode:form.value.forwardMode, params:form.value.params, inputExample: form.value.inputExample, outputExample: form.value.outputExample }
...
async function buildPreview(list) {
  ...
  importPreview.value = list.map(s => ({
    ...
    exists: allSlugs.has(s.slug), selected: true, welcomeUrl: s.welcomeUrl || '',
  }))
}
...
async function doImport(strategy) {
  try {
    const data = importPreview.value.map(s => ({ slug: s.slug, name: s.name, description: s.description, category: s.category, targetUrl: s.targetUrl, welcomeUrl: s.welcomeUrl, method: s.method, forwardType: s.forwardType || 'json', forwardMode: s.forwardMode || 'template', customScript: s.customScript || '', params: s.params, bodyTemplate: s.bodyTemplate, forwardHeaders: s.forwardHeaders, inputExample: s.inputExample, outputExample: s.outputExample }))
...
.row2 .fg label{min-height:1.4em}
...
<style>
/* 编辑弹窗 - 全局样式（el-dialog teleported） */
