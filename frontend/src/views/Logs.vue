<template>
  <div class="page container">
    ...
    <div class="card"><div class="cb">
      <table v-if="logs.length"><thead><tr><th>时间</th><th>方法</th><th>路径</th><th>服务</th><th>状态码</th><th>响应时间</th><th></th></tr></thead><tbody>
        <template v-for="l in logs" :key="l.id">
          <tr class="lr" :class="{ex:l._e}" @click="l._e=!l._e"><td class="time">{{ l.timestamp }}</td><td><span class="mt">{{ l.method }}</span></td><td class="path">{{ l.path }}</td><td>{{ l.serviceName || l.serviceSlug || '-' }}</td><td><span :class="sc(l.statusCode)">{{ l.statusCode }}</span></td><td>{{ l.responseTime }}ms</td><td><span class="ei">{{ l._e?'▲':'▼' }}</span></td></tr>
          <tr v-if="l._e" class="ldr"><td colspan="7"><div class="ld">
...
const fl = reactive({ serviceSlug:'', status:'', startTime:'', endTime:'' })
...
.path { max-width: 220px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-family: 'Consolas',monospace; }
