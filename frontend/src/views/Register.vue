...
<script setup>
import { ref } from 'vue'; import { useRouter } from 'vue-router'; import { useAuthStore } from '@/stores/auth'; import { useToastStore } from '@/stores/toast'
const r = useRouter(); const auth = useAuthStore(); const toast = useToastStore()
const f = ref({ username: '', email: '', password: '' }); const ld = ref(false); const err = ref('')
async function go() { err.value = ''; if (!/^[a-zA-Z0-9_]{3,30}$/.test(f.value.username)) { err.value = '用户名格式不对'; return }; ld.value = true; try { await auth.register(f.value); toast.success('注册成功'); r.push('/dashboard') } catch(e) { err.value = e.message } finally { ld.value = false } }
</script>
<style scoped>
.fg{margin-bottom:16px}.fg label{display:block;margin-bottom:6px;font-weight:500;font-size:.875rem}
.fc{width:100%;padding:10px 12px;border:1px solid #e2e8f0;border-radius:8px;font-size:.875rem;outline:none}.fc:focus{border-color:#4f46e5}
